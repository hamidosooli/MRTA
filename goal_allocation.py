    ...
    for robot in robot_list:
        ...
        for victim_id in robot.tasks_init:
            for cap in victim_list[victim_id].capabilities:
                if cap in robot.capabilities:  # Add the time for the accomplished task to the robot's make span
                    robot.make_span.append(victim_list[victim_id].make_span[cap])
                else:  # Accumulate the robot's abort time for the tasks he couldn't accomplish in the cluster
...
def time_cost(robot_list, victims_new, alpha=.2, beta=.25, gamma=.55):
    ...
    cost = np.zeros((num_robots, num_rem_tasks))

    for robot in robot_list:
        for victim_id, victim in enumerate(victims_new):
            for cap in victim.capabilities:
